...
??? info "Ders akışı (öğretim üyesi için zaman planı)"
    ...
    | 0:10–0:30 | 2 | SEI CERT C/C++: kuralların yapısı ve en önemli kurallar |
    | 0:30–0:50 | 3 | Biçim dizisi açığı — **Demo 1** |
    | 0:50–1:00 | Ara | |
    | 1:00–1:20 | 4–5 | Serbest bırakılmış bellek **Demo 2**; tamsayılar ve tanımsız davranış **Demo 3** |
    | 1:20–1:35 | 6 | Hata işleme, değişken argümanlar ve sinyaller |
    | 1:35–1:50 | 7–8 | Sanitizer'lar; fuzzing'e giriş — **Demo 4** |
    | 1:50–2:00 | Ara | |
    | 2:00–2:25 | 9 | Derleyici ve işletim sistemi korumaları — **Demo 5** |
    | 2:25–2:50 | 10–12 | Kod gizlemeye giriş; sembol, dize ve günlük **Demo 6**; kontrol akışı düzleştirme **Demo 7** |
    | 2:50–3:00 | 13+ | Proje adımı, kendini sınama |
...
1. haftada uygulama korumasının yedi katmanını gördük. Bu hafta bunlardan üçünü, C ve C++ kodu özelinde açıyoruz:
...
| ERR33-C | Kütüphane hatalarını algıla | Bu hafta 6. bölüm |
| SIG30-C | Sinyal işleyicide yalnız güvenli fonksiyonlar | Bu hafta 6. bölüm |
...
## 3. Biçim dizisi açığı (Tarif 3.2)
...
## 4. Serbest bırakılmış belleğin kullanımı ve çift serbest bırakma

1. haftada bellek yönetimi hatalarının kuramını ve sahiplik kuralını gördük. Bu bölümde aynı hatayı çalışan bir
...
## 5. Tamsayılar ve tanımsız davranış (Tarif 3.5)

1. haftada işaretli bir uzunluğun `size_t`'ye dönüşünce dev bir sayıya dönüştüğünü gördük. Bu bölümde tamsayı
...
## 6. Hata işleme, değişken argümanlar ve sinyaller (Tarif 13.1, 13.4, 13.5)
...
kullanıyorsanız 3. bölümdeki `format` özniteliğini ekleyin. C++'ta değişken argüman yerine tür güvenli şablonlar
...
## 7. Sanitizer'lar: hataları çalışma anında yakalamak
...
## 9. Derleyici ve işletim sistemi korumaları
...
!!! tip "Önerilen sürüm bayrakları"
    **GCC/Clang:** `-O2 -D_FORTIFY_SOURCE=2 -fstack-protector-strong -fPIE -pie -Wl,-z,relro,-z,now
    ...
    /DYNAMICBASE /HIGHENTROPYVA /NXCOMPAT /CETCOMPAT /W4`. Dersin CMake yapısındaki `guvenli` kipi bu bayrakları
    sizin yerinize açar (`code/cmake/cen429.cmake`). OpenSSF'nin "Compiler Options Hardening Guide for C and C++"
...
## 10. Kod gizlemeye giriş (Tarif 12.1, 12.3)
...
## 11. Sembol, dize ve günlük gizleme
...
## 12. Kontrol akışı düzleştirme ve opak değerler (Tarif 12.3, 12.8)
...
Güvenlik kılavuzunuzun **S6 "Kod sağlamlaştırma"** bölümünün ilk taslağını yazın:

- [ ] Projenizi dersin `guvenli` kipindeki bayraklarla derleyin; koruma tablosunu (`checksec` / `dumpbin`) kılavuza
      ekleyin. Kapalı kalan bir koruma varsa gerekçesini ödünleşim kaydına yazın.
...
```markdown title="S6 önlem kartı şablonu (her önlem için)"
...
??? question "Alıştırma 6 — Orta: Koruma tablosu"
    Aynı programı dersin `korumasiz` ve `guvenli` kiplerinde derleyin; iki ikili dosyanın koruma tablosunu çıkarın.
    Hangi bayrak hangi satırı değiştirdi? İkili dosyaların boyutu ve çalışma süresi ne kadar farklı?
...
??? question "Alıştırma 8 — Zor: Düzleştirmenin maliyeti"
    ...
    dosya boyutunu ölçün; sonucu S6 bölümüne bir ödünleşim satırı olarak yazın.
