...
# Girdi doğrulama: girdi nereden gelir? (Tarif 3.1)

| Kaynak | Sık unutulan |
| --- | --- |
| Komut satırı, ortam | `argv[0]` bile saldırganın; `PATH`, `LANG` |
| Dosya | Biçimdeki uzunluk alanları |
| Ağ | "16 bayt" der, 1 bayt gönderir |
| IPC | Aynı makinedeki süreç de güvenilmez |
| Veritabanı | "Kendi verimiz" sonradan değişmiş olabilir |
| Kütüphane arayüzü (JNI) | Çağıran uygulama da güvenilmez |

---

# Beş ilke

1. **Beyaz liste**, kara liste değil
2. Önce **kanonikleştir**, sonra doğrula (`/tmp/../etc`, `%2e%2e/`)
3. **Uzunluk → tür → aralık → biçim**
...
`atoi("abc")` = 0 · `atoi("99999999999")` = tanımsız · `atoi("12abc")` = 12

---

<!-- _class: yogun -->

# Uzunluk önekli kayıt: iki denetim
...
- Kaynak ve hedef **ayrı ayrı** · denetim **sırası** önemli (`kalan - 3` sarmasın)
...
# CERT çiftleri (1): STR31-C · INT30-C
...
# CERT çiftleri (2): MEM30-C · EXP33-C · ERR33-C

```c
for (d = bas; d; d = d->sonraki) free(d);            /* HATALI: free'den sonra d->sonraki */
while (d) { Dugum *s = d->sonraki; free(d); d = s; } /* UYUMLU */
```

```c
int sonuc;            if (k) sonuc = f(); return sonuc;   /* HATALI */
int sonuc = HATA_KODU; if (k) sonuc = f(); return sonuc;  /* UYUMLU: güvenli varsayılan */
```

```c
FILE *f = fopen(yol, "rb"); fread(t, 1, n, f);            /* HATALI */
if (!f) return HATA; if (fread(...) < n && ferror(f)) ... /* UYUMLU */
```

Her bulguyu bir kural kimliğiyle eşleştir → nesnel ve aranabilir
...
# 4. Statik analiz, sanitizer'lar ve fuzzing
...
| Düzey | Araç | Bulduğu |
| --- | --- | --- |
| Uyarılar | `-Wall -Wextra -Wformat=2 -Wconversion` · `/W4` | Dönüşüm, biçim dizgesi |
| Derleyici analizcisi | `gcc -fanalyzer` · `clang --analyze` · `/analyze` | NULL, sızıntı, çift bırakma |
| Kural denetleyici | `clang-tidy` (`cert-*`), `cppcheck --addon=cert` | CERT ihlalleri |
| Anlamsal sorgu | CodeQL, Semgrep | Kaynak → hedef veri akışı |
| Ticari | Coverity, Klocwork | Derin analiz, raporlama |

Statik: çalışmayan yolları da görür, yanlış alarm verir · Dinamik: kesin ama yalnız çalışan yol
...
2. Her bastırma **gerekçeli**: `// NOLINT(cert-err33-c): ...`
3. Yeni kodda **sıfır uyarı** (CI zorlar)

<!-- Konuşma notu: Değerlendirici geliştiricinin kendi tarama raporunu da ister; ardından aracın göremediği kripto kullanımı ve güvenlik denetimi mantığına elle bakar. -->
...
Demo 5 `giris_sert` = bu bayrakların çoğu · kendi projenize siz eklersiniz · OpenSSF *Compiler Options Hardening Guide*
...
```text
PR → derle (-Werror) → statik analiz → testler (ASan+UBSan) → kısa fuzzing
   → sürüm derlemesi (korumalar açık) → ikili denetimi (checksec, strings) → birleştir
```

| Aşama | Başarısızlık ölçütü |
| --- | --- |
| Derleme | Tek bir uyarı |
| Sanitizer'lı test | Herhangi bir rapor |
| Fuzzing | Yeni çökme · eski corpus = **regresyon testi** |
| İkili denetimi | Günlük dizgesi, silinmemiş semboller, kapanmış koruma |

---

<!-- _class: yogun -->

# Dersin CMake kipleri ve tedarik zinciri

| Kip | Bayraklar |
| --- | --- |
| `korumasiz` | `-O0` / `/Od` |
| `optimize` | `-O2` / `/O2` |
| `asan` | `-O1 -fsanitize=address` (Linux'ta FORTIFY kapalı) |
| `denetimli` | `-O2 -D_FORTIFY_SOURCE=2` |
| `guvenli` | `-O2` + **düzeltilmiş kaynak** |

Koruma bayraklarının çoğu: Demo 5 `giris_sert`

Derleme ortamı da hedef: erişim kaydı · bağımlılık sabitleme (5. hafta SBOM) · imzalı sürüm · yeniden üretilebilir derleme
...
# Proje: S9 "Kod sağlamlaştırma" taslağı
...
6. `giris_zayif` vs `giris_sert`: koruma tablosu; sonra kendi projene uygula
