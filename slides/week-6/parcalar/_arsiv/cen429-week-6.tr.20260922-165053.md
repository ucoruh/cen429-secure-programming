...
| Saat | Konu |
| --- | --- |
| 1 | RASP nedir · **RASP mimarisi** · bütünlük denetimi · anti-debug · **Demo 1–2** |
| 2 | Ortam/emülatör · kanca/preload · root · imza · **Demo 3–4, 6–7** |
| 3 | **Dinamik bellek koruması** · kontrol akışı sayacı · tepki politikası · cihaz bağlama · **Demo 5, 8** |
...
| An | Örnek | Neden? |
| --- | --- | --- |
| **Yükleme** | Kütüphane belleğe yüklenince | Ortam baştan güvenilmezse hiçbir sır açılmaz |
| **Hassas işlem öncesi** | Anahtar çözmeden, ödemeden, kimlik doğrulamadan önce | Hata ayıklayıcı açılıştan **sonra** bağlanabilir |
| **Arka plan, düzensiz** | Ayrı iş parçacığı, rastgele zaman | Sabit zamanlı denetimin "arasına" girilir |

Yalnız açılışta yapılan denetim, sonradan bağlanan hata ayıklayıcıyı **görmez**

---

# Nerede çalışır? Dağıtık denetim

- **Çok noktalılık:** aynı denetim birçok yerde, her seferinde biraz farklı
- **Karşılıklı denetim:** Java ↔ native birbirinin bütünlüğünü doğrular
- **Korunan fonksiyonun içinde:** sahada tek bir değerli fonksiyon kendi giriş noktasında ~15 denetimle korunur
- **Sahte parametreler:** giriş noktasında bütünlük denetimini tetikleyen ek parametre (4. hafta)

---

<!-- _class: yogun -->

# Nasıl karar verir? Veri bağımlı, merkezî değil

```c
if (hata_ayiklayici_var() || root_var() || butunluk_bozuk())
    return HATA;           /* tek satırı değiştiren her şeyi atlatır */
anahtari_coz();
```

- Sonuç bir `if`'e değil **sonraki hesabın verisine** karışsın: temiz → doğru anahtar, değil → geçerli görünen **yanlış** anahtar
- Sonuçlar **opak** değerlerle taşınsın (4. hafta)
- Tepki tetikleyiciden **uzak** (tepki politikası)
...
| Katman | Kapattığı açık |
| --- | --- |
| Gizleme | Denetimleri **bulmayı** zorlaştırır |
| Bütünlük denetimi | Denetimlerin **yamalanmasını** fark eder |
| Hata ayıklayıcı / kanca / ortam | Çalışma anı **analizini** fark eder |
| Kontrol akışı sayacı | Denetimlerin **atlanmasını** fark eder |
| Cihaz / sürüm bağlama | Kopyalanan veriyi **işe yaramaz** kılar |
| Sunucu tarafı risk motoru | İstemci tamamen ele geçse bile **son kararı** verir |

<!-- Konuşma notu: Şema gereksinimleri tam bu katmanları sayar: değişikliğe karşı koruma, öz bütünlük, desteklenmeyen cihazda çalışmama, geri almayı engelleme, tersine mühendislik ve kancaya karşı savunma, hata ayıklama/test ortamı algılama, uzlaşmada bildirim ve silme. Sertifikalı kütüphanede native 14 + Java 14 RASP önlemi. -->
...
# 3. Bütünlük denetimi: uygulama kendini doğrular
...
# 6. Kanca ve enstrümantasyon (LD_PRELOAD, Frida)
...
| Saldırı | Örnek | Hedef |
| --- | --- | --- |
| Okuma | Döküm al, bilinen değeri ara | Anahtar, PIN, çözülmüş veri |
| Değiştirme | Sayaç, puan, "lisans geçerli" bayrağı | Mantık ve denetimler |
| İzleme | Değer değişince haber veren araçlar | Değişkenlerin yerini bulmak |

Hepsi için belleğe **erişim** gerekir → hata ayıklayıcı ve kanca algılamayla birlikte düşünülür

---

<!-- _class: yogun -->

# Önlem 1–2: az, kısa, dağınık · işletim sistemiyle zorlaştır

1. **Kısa ömür:** kullanıldığı fonksiyonda açık, dönmeden sil
2. **Örtme / parçalama:** maskeli ya da parçalı dur
3. **Rastgele değerle sil:** "burada sır vardı" izi de kalmasın
4. **Hiç açılmasın:** whitebox (11. hafta)

| OS önlemi | Etkisi |
| --- | --- |
| `PR_SET_DUMPABLE = 0` | Aynı kullanıcı bile bağlanamaz, belleği okuyamaz |
| `mlock` / `VirtualLock`, `MADV_DONTDUMP` | Takas ve döküme düşmez |
| `mprotect` / `VirtualProtect` | Değişmemesi gereken tablo **salt okunur** |

Root'u durdurmaz ama saldırganı **daha güçlü** konuma zorlar
...
typedef struct {
    ...
    uint32_t ozet;      /* ikisi üzerinden kısa MAC       */
} KorunanSayac;
/* okurken üçü tutarlı mı? değilse bellekte değiştirilmiş */
```

- Yalnız `deger` değişirse **tutmaz** → maske ve MAC anahtarını da bulmak gerekir
- Aynı fikir: 5. hafta ayrık parmak izi · 2. hafta kurcalamaya dayanıklı günlük
- ⚠️ Tutarsızlık bir **tepkiye** bağlanmalı; denetim kodu da bütünlük kapsamında

---

# Önlem 4: izlendiğini fark et

- Temel: hata ayıklayıcı ve kanca algılama
- **Zamanlama:** tek adım / kesme noktası → beklenmedik uzun süre
- **Beklenmeyen modüller:** uygulamanın yüklemediği kütüphaneler
- **Tutarlılık bozulması:** korunan değerler tutmuyor

Hiçbiri tek başına kesin değil → **risk skoruna** katkı

<!-- Konuşma notu: Değerlendirici hassas işlem sırasında ve sonrasında döküm alıp test sırlarını arar; bir sayacı bellek düzenleyiciyle değiştirip uygulamanın fark edip etmediğine bakar. Rapor: sır bellekte ne kadar açık kaldı, değiştirilen değer fark edildi mi. -->
...
# 8. Root / ayrıcalık ve bileşen imzası
...
# 10. Tepki politikası, cihaz bağlama, caydırma
...
- **S10 — RASP algılama envanteri:** her kritik işlem × kontroller (neyi algılar,
  nasıl atlatılır, nasıl güçlendirilir)
- **S10 — Tepki politikası:** hangi sır silinir, fail-closed/decoy, cihaz bağlama,
  sunucu bildirimi
